...
func TestProjectColorCoversWholePalette(t *testing.T) {
	...
	for _, name := range domain.NodeColors {
		if got := kindcolor.NodeColor(name, p); got == p.FgMuted {
			t.Errorf("color %q resolved to the muted fallback", name)
		}
	}
	if kindcolor.NodeColor("", p) != p.FgMuted {
		t.Error("empty name → FgMuted")
	}
	if kindcolor.NodeColor("chartreuse", p) != p.FgMuted {
...
func TestProjectColorMapsKnown(t *testing.T) {
	p := theme.Default
	if kindcolor.NodeColor("blue", p) != p.Blue {
		t.Error("blue must map to p.Blue")
	}
	if kindcolor.NodeColor("teal", p) != p.Teal {
