...
// --- Free (node-less) verbs (free-artifacts Task 3) ------------------------

func TestUploadFreeArtifact_PostsToFreePath(t *testing.T) {
	want := stubArtifact("", "brand")
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/api/v1/artifacts" {
			t.Errorf("path: got %s, want /api/v1/artifacts", r.URL.Path)
		}
		var body map[string]string
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
			t.Fatalf("decode body: %v", err)
	...
	c := apiclient.New(srv.URL, "tok")
	got, err := c.UploadFreeArtifact(context.Background(), "Diagram.png", "application/octet-stream", data)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if got.Slug != want.Slug || got.Mime != want.Mime {
...
func TestListFreeArtifacts_GetsFreePath(t *testing.T) {
	items := []domain.Artifact{stubArtifact("", "a"), stubArtifact("", "b")}
	...
	c := apiclient.New(srv.URL, "tok")
	got, err := c.ListFreeArtifacts(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(got) != 2 {
...
func TestDeleteFreeArtifact_SendsDeleteToFreePath(t *testing.T) {
	...
	c := apiclient.New(srv.URL, "tok")
	if err := c.DeleteFreeArtifact(context.Background(), "diagram"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if gotMethod != http.MethodDelete {
		t.Errorf("method = %q, want DELETE", gotMethod)
	}
	if gotPath != "/api/v1/artifacts/diagram" {
		t.Errorf("path = %q, want /api/v1/artifacts/diagram", gotPath)
	}
}
