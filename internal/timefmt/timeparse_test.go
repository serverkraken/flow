...
func TestParseHM(t *testing.T) {
	...
	if _, err := timefmt.ParseHM("nope"); err == nil {
...
func TestParseStop(t *testing.T) {
	...
	// absolute HH:MM
	got, err := timefmt.ParseStop("12:00", start, now)
	if err != nil {
	...
	got2, err := timefmt.ParseStop("+1h30m", start, now)
	if err != nil {
...
func TestNormalizeDurationArg(t *testing.T) {
	...
	for _, c := range cases {
		if got := timefmt.NormalizeDurationArg(c.in); got != c.want {
