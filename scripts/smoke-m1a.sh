...
echo "== /healthz =="
curl "${insecure[@]}" -fsS "$BASE/healthz" && echo

echo "== /api/v1/me without token (expect 401) =="
code=$(curl "${insecure[@]}" -s -o /dev/null -w '%{http_code}' "$BASE/api/v1/me")
[ "$code" = "401" ] && echo "OK 401" || { echo "FAIL: $code"; exit 1; }

echo "== / without session cookie (expect 302 -> /auth/login) =="
loc=$(curl "${insecure[@]}" -s -o /dev/null -w '%{redirect_url}' "$BASE/")
...
== authed live-sync (manual / scripted) ==
  ...
  WebUI:  open https://localhost:8080/  -> login via Authentik -> start a timer
