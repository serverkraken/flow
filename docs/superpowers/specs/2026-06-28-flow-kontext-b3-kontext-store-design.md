...
**Datum:** 2026-06-28 · **Branch:** `b3-kontext-store` (von `rebuild` @ `794bd59`, noch **nicht integriert**) — wie B1/B2 · **Status:** **IMPLEMENTIERT** 2026-06-29 (Slices A–D1, 21 commits + final-review-Fix `3a1d5ee`; subagent-driven, 15 Tasks je per-Task-Review + Opus-Whole-Branch-Review = ready-to-merge nach dem einen Fix: `handlePinDocument`/`handlePutContextActive` publishen jetzt `document.updated` SSE wie alle anderen Doc-Writes). `make ci` grün (lint 0, Coverage 76.2% ≥ 75%, Binaries bauen). **Live-Smoke vs Postgres+Dex grün** (Migration 0021 angewandt; `GET /context` unresolved→200 `{cap:6000}`; `PUT /context/active`→200; resolved `GET ?node=`→Ahnenkette `chain_len 2` + activeContext extrahiert + Budget; `POST …/pin`→204). **Offen (Soenne, interaktiv):** echter Claude-Code-Hook-Dogfood (SessionStart lädt / Stop erinnert bei stale+Arbeit / Offline-Fallback bricht Start nicht). **Review-Minor #3:** Relevanz-Rang ist 2-Key (`pinned` → `updated_at` desc); D7-Tag-Match ist das *Gate* für global-Eligibilität, **kein** Sort-Key — engagement- + gegatete global-Memories werden unter Budget bewusst nur nach Neuheit gerankt.
