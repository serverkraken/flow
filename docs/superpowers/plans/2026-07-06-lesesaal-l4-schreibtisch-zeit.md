...
| 2 Schreibtisch-Rebuild (Jetzt/Weiterarbeiten/Wissen/Puls; MRU-Builder; pulseRow- + NodeAvatar-Extraktion; Retirement) | `lesesaal-implementer-deep` | Sonnet · high |
| 3 Zeit-Hub-Rebuild (Ledger/Wochenskala/Werkzeuge; Subnav weg) | `lesesaal-implementer-deep` | Sonnet · high |
| 4 Werkzeuge Woche — **voller Lesesaal-Rebuild** + Statistik-Destination | `lesesaal-implementer-deep` | Sonnet · high |
| 5 Werkzeuge Historie — **voller Lesesaal-Rebuild** (Kalender/Liste/Reassign) | `lesesaal-implementer-deep` | Sonnet · high |
| 6 Werkzeuge Frei + Export — **voller Lesesaal-Rebuild** | `lesesaal-implementer-deep` | Sonnet · high |
| 7 Asset-Fingerprinting + Cache-Control (`/static/` — PROD-Cache-Bug) | `lesesaal-implementer-deep` | Sonnet · high |
| 8 Wiring-Gate (Leichen-Sweep · tote Keys · make ci · Live-Smoke · Breakpoints) | `lesesaal-implementer` | Sonnet · medium |
...
- Modify: `internal/adapter/webui/components/avatar.templ` (neue **logo-aware `NodeAvatar`**-Komponente) + `internal/adapter/webui/cockpit_head.templ` (Cockpit-96er-Identität von der Inline-Logo-Logik auf `NodeAvatar` umstellen — **Single Source**; behebt zugleich Soennes Beobachtung 06.07., dass Logos nur im Cockpit erscheinen — siehe Offene Entsch. #12)
...
**Interfaces:**
- **`webui.RecentNode` + `BuildRecentNodes(sessions []domain.WorkSession, nodes []domain.Node, now time.Time, n int) []RecentNode`** (home_continue.go, domain-frei, unit-getestet). MRU aus Sessions wie die Palette (`rg -n "recent|MRU|last 30|ws.NodeID" internal/adapter/httpserver/webui_palette.go` — Muster verifizieren): distinkte **bookable** Knoten, nach jüngster Session absteigend, cap `n` (5). Felder: `{ ID, Name (ShortName), FullPath, Tone (AvatarTone), Initials, ValueStr (z. B. „2:41 h" laufend / „gestern" / letzte Session-Dauer), LabelKey/LabelStr (z. B. „läuft gerade"/„zuletzt aktiv") }`. **Kein neues Schema** — nur Ableitung. Kurzname/Pfad über `ShortName`/den Knoten-Pfad (verifizieren, wie L2 den vollen Mono-Pfad rendert: `rg -n "FullPath|\.path|Pfad" internal/adapter/webui/nodes.templ`). **Zusätzliches Feld `LogoRef string`** (aus `domain.Node.LogoRef`, verifiziert node.go:42 — Content-Hash, „" = kein Logo) für logo-aware Avatare.
- **`NodeAvatar(nodeID, logoRef, initials, tone, sizeClass string)`** in `components/avatar.templ` (neu): rendert `<img src="/nodes/{nodeID}/logo?v={logoRef}" class="…object-contain…">` im `ava ava-logo {size}`-Span **wenn `logoRef != ""`**, sonst die Initialen-`Avatar` (Bestand). **Warum:** die Bestand-`components.Avatar` ist initialen-only (ihr eigener Kommentar versprach eine „NodeLogo"-Logo-Variante, die nie als wiederverwendbare Komponente extrahiert wurde); nur `cockpit_head.templ` rendert heute Logos, inline (`if d.N.LogoRef != "" → <img …/logo?v=…>`, sonst `Avatar(…, "av-96")`). Diese Inline-Logik wird **wörtlich** nach `NodeAvatar` gehoben und das Cockpit ruft danach `NodeAvatar` — eine Quelle (Spec §8: Logo-oder-Initialen überall, Größen 28/36/56/96). Die Schreibtisch-„Weiterarbeiten"-Zeilen (Mockup Z.382 zeigt dort ein Logo!) und die Jetzt-Zeile nutzen `NodeAvatar` statt `Avatar`. `rg -n "d.N.LogoRef|ava-logo|/logo\?v=|cockpitIdentity" internal/adapter/webui/cockpit_head.templ` vor dem Extrahieren.
...
- **`home.templ`-Rebuild:** `HomePage`/`homeBody`/`homeOuter`/`HomeFragment`-Signaturen **stabil** (Handler-Caller unberührt). `homeOuter` `#content` `hx-get="/ui/home"` `hx-trigger="sse:session.started, sse:session.stopped, sse:session.updated, sse:session.deleted, sse:document.created, sse:document.updated, sse:document.deleted, sse:dayoff.changed, sse:activity.logged, sse:settings.changed"` `hx-swap="innerHTML"` (**`settings.changed` behalten** — Bestand-Parität, Gemini-Fund #4; harmlos, kein settings-abhängiger Wert mehr auf Home, aber kein stiller Trigger-Verlust). `HomeFragment` rendert:
  ...
  - **Jetzt** (`.sect` + `.sect-h`: `.eyebrow`=„Jetzt" + `.more` „Zeit ›"→`/zeit`): läuft → `.row.panelrow` (`NodeAvatar`(Logo/Initialen) `av-36` + `.grow`(„Timer läuft auf" + `.targetlink`→`NodeHref`; `.s`=`SinceStr` + Work/Privat) + `.livechip` + `.right`(`<span data-timer data-timer-fmt="clock" data-base={BaseSeconds}>` + `.k`=„heute {TodayLogged}") + **Stop nur bei `Now.NodeID != ""`** (gebundener Timer): `.btn.btn-q.btn-s` `hx-post="/ui/timer/stop" hx-swap="none"` (Antwort verworfen; Pill + `#content` ziehen per SSE nach — **NICHT** in `#timer-pill` swappen; Begründung im Timer-Vorgabe-Block)). Idle → **eine ruhige `.row.panelrow`** ohne Start-CTA: „Kein Timer läuft" + `.k`=„heute {TodayLogged}" (Offene Entsch. #1/#2).
  - **Weiterarbeiten** (`.sect` + `.sect-h` „Weiterarbeiten" + `.more` „Alle Projekte ›"→`/nodes`): für jeden `RecentNode` ein `<a class="row" href="/nodes/{ID}">` (`NodeAvatar`(LogoRef→Logo, sonst Initialen) `av-36` · `.t`=Name · `.path`=FullPath · `.right .v`=ValueStr · `.k`=LabelStr). Leer → ruhige Zeile „Noch keine Aktivität". (Mockup Z.382 zeigt hier explizit ein Logo-Avatar.)
...
- [ ] **Step 3: `home_continue.go` implementieren** + `pulseRow`/`pulseSection` promoten + `cockpit_main.templ` umstellen (`cockpitPulseRow`→`pulseRow`) + **`NodeAvatar` extrahieren** (Logo-oder-Initialen aus `cockpit_head.templ` in `avatar.templ` heben; Cockpit-Identität auf `NodeAvatar` umstellen — Cockpit-Render-Test grün halten). **Offene Entsch. #12 = ENTSCHIEDEN „ja" (Soenne 06.07.):** in `nodes.templ` die zwei `components.Avatar`-Aufrufe (Engagement-Header av-36, Repo-Zeilen av-28) auf `NodeAvatar` umstellen und `LogoRef` in die Node-Listen-VM(s) threaden (`rg -n "components.Avatar|Initials|Tone|type .*Row" internal/adapter/webui/nodes.templ internal/adapter/webui/nodes_vm.go`; der Handler muss `node.LogoRef` in die Zeilen-VM übernehmen) → Logos erscheinen auch auf `/nodes`. **Owner-Scope/Render-Test:** eine `/nodes`-Assertion, dass ein Knoten mit `LogoRef` das `<img …/logo?v=…>` rendert und einer ohne die Initialen; Kristall-Assertions unangetastet.
...
```
(Bestehende `home.greeting`/`home.activity`/`activity.on`/`activity.empty`/`nav.*` prüfen/wiederverwenden. `stats.tileToday/Week/Month`, `heute.legend*`, `activity.filter.*`, `activity.actor.all` → Task 8 prüft, ob nach der Umstellung tot.)
...
> **Werkzeuge-Destinationen = TIEFER Rebuild (Soenne-Entscheidung 06.07.):** Woche, Historie, Frei und Export werden **voll auf Lesesaal-Sprache neu gesetzt** (Pfad-Rückgrat/pagehead, Zwei-Flächen-Regel, Haarlinien-Zeilen statt Karten, benannte Klassen) — **kein** Minimal-Restyle. Die Kristall-Templates (`components.Card`/`glass`/`shadow`/`font-display`/`rounded-3xl` + die Kristall-Render-Helfer) sterben; die **VMs/Handler/Usecases/Routen/JS bleiben** (Daten + Verhalten unverändert), nur die Präsentation wird neu gebaut. Deshalb drei eigene Deep-Tasks (kein Monolith), je failing-Test-first.

### Task 4: Woche — voller Lesesaal-Rebuild + Statistik-Destination (Burndown/Saldo-Umzug)

**Files:**
- Modify: `internal/adapter/webui/woche.templ` (voller templ-Neubau) + `internal/adapter/webui/woche_vm.go` (Kristall-Render-Helfer entfernen/ersetzen)
- Modify: `internal/adapter/httpserver/webui_woche.go` (Burndown/Saldo-Metrik hierher, Offene Entsch. #4)
- Modify: `internal/i18n/catalog_de.go` + `catalog_en.go`
- Test: `internal/adapter/httpserver/webui_woche_test.go`, `internal/adapter/httpserver/webui_worktime_handlers_test.go`, `internal/adapter/webui/woche_vm_internal_test.go`

**Interfaces:** **Kein Verhaltens-/Routen-Change** (KW-Nav prev/next/this-week, Wochendaten bleiben; `wocheDataFor`/`WocheVM`/`WocheDayVM` unangetastet als Datenquelle). Präsentation neu:
- **Lesesaal-Kopf statt Sub-Tab-Strip:** `AppShell("zeit", nil, nil, …)`; `.pagehead` (`.eyebrow`=„Zeit / Statistik", `h1`=`KWLabel`+`WeekLabel`, `.sub`) + **„‹ Zeit"-Rücklink** (`.spine .up`-Muster: „‹ Zeit" → `/zeit`). KW-Nav (prev/`Diese Woche`/next) als Lesesaal-Controls (`.btn-q.btn-s`, `CanForward` steuert next), htmx auf `/ui/woche/fragment`.
- **Woche als Inhalt:** die vertikale **`.weekbar`** (Task 1, konsistent mit dem Zeit-Hub) als Skyline **plus** eine Mo–So-`.row`-Liste (`.led-when`/Label · `.grow`(Datum · Saldo-Zeile) · `.right`(`.v`=Dauer, `.k`=Ziel)) — Haarlinien statt Karten. DayOff/Feiertag als `.typechip`/`tc-*`-Ton in der Zeile (kein Kristall-`wocheDayOffChip`). **Retire** die Kristall-Render-Helfer `wocheBarFill`/`wocheDayDotClass`/`wocheDayDotGlyph`/`wocheLabelClass`/`wocheSaldoHue`/`wocheDayOffChip` (nur woche-lokal — `rg -n "wocheBarFill|wocheDayDotClass|wocheDayOffChip" internal/ -g '!*_templ.go'`), soweit der Neubau sie ersetzt; `wocheDayBarStyle`/`wochePctStr`/`ClampPct` (Datenbindung) bleiben ggf.
- **Kennzahlen als Zwei-Flächen-Panels:** `WocheVM.Total` (WeekTotalVM) + `WocheVM.Kennzahlen` (KennzahlenVM) als `.panel` mit `.krow`-Zeilen (Soll/Ist/Saldo/Ø-Tag …) statt `components.Card`. VM-Felder exakt: `rg -n "type WeekTotalVM|type KennzahlenVM" internal/adapter/webui/components/*.go`.
- **Statistik-Rolle (Offene Entsch. #4/#5, DECIDED = Umzug):** die monatliche **Burndown/Saldo-Metrik** (die von Home fällt) hier als **Lesesaal-`.panel`-Kennzahlenblock neu bauen** (`s.Stats.Burndown` — `rg -n "Stats.Burndown|burndownBannerVM|BurndownVM" internal/adapter/httpserver internal/adapter/webui`), **nicht** den Kristall-`BurndownBanner` wiederverwenden. So bleibt die Glance-Metrik erhalten und lebt am richtigen Ort (Statistik).

**Zustände:** leer (`WocheVM.Empty` → ruhige „Keine Zeit erfasst"-Zeile, `.weekbar` mit `—`-Tagen), lang (Node-Namen `ShortName`+`title`), mobil 375px (`.weekbar` scrollt `overflow-x:auto`, Kennzahlen-Panels stapeln), laufender Timer (Woche „heute"-Balken `--live-bright`; unbeteiligt sonst), Fehlerpfad (500 wie Bestand; KW in der Zukunft → `CanForward=false`).

- [ ] **Step 0: rg-Verifikation** — `rg -n "func .*wocheDataFor|WocheVM|WocheDayVM|WeekTotalVM|KennzahlenVM|wocheBarFill|wocheDayDotClass|wocheDayOffChip|handleWoche" internal/adapter/webui internal/adapter/httpserver -g '!*_templ.go'` · `rg -n "Stats.Burndown|BurndownVM|burndownBannerVM" internal/adapter -g '!*_templ.go'` · `rg -n "worktimeSubnav" internal/adapter/webui -g '!*_templ.go'` (Historie ist noch Aufrufer — erst Task 5 löscht die Definition; hier NICHT löschen).
- [ ] **Step 1: Failing Test** — `webui_woche_test.go`: Woche-Fragment enthält `.pagehead`/`.weekbar`/`.panel`/`.krow`/`.spine`, **kein** `glass`/`components.Card`/`shadow-soft`/`font-display`, den Burndown-Kennzahlenblock; KW-Nav (`?week=`) antwortet 200. Owner-Scope bleibt (Bestand-Negativtest prüfen/ergänzen).
- [ ] **Step 2: Laufen lassen** — FAIL.
- [ ] **Step 3: `woche.templ`-Neubau + `woche_vm.go`-Helfer-Retire + Burndown-Panel** (`webui_woche.go`); `worktimeSubnav` NICHT anfassen (Historie-Aufrufer). i18n neue Keys (`zeit.back` „‹ Zeit", `woche.statsTitle`, Burndown-Labels) in beide Kataloge.
- [ ] **Step 4: Bauen + Tests + Commit**
```bash
make generate && make web && go test ./internal/adapter/webui/... ./internal/adapter/httpserver/... -race 2>&1 | tail -20
git add -A && git commit -m "feat(lesesaal): Woche voll auf Lesesaal neu gesetzt — Statistik-Destination (Weekbar + Kennzahlen-Panels + Burndown-Umzug), Kristall raus"
```

---

### Task 5: Historie — voller Lesesaal-Rebuild (Kalender + Session-Liste + Reassign/Bulk-Delete)

**Files:**
- Modify: `internal/adapter/webui/historie.templ` (voller templ-Neubau) + `internal/adapter/webui/historie_vm.go` (Kristall-Render-Helfer entfernen/ersetzen)
- Modify: `internal/adapter/webui/heute.templ` (die `worktimeSubnav`-**Definition** jetzt entfernen — letzter Aufrufer fällt hier)
- Modify: `internal/i18n/catalog_de.go` + `catalog_en.go`
- Test: `internal/adapter/httpserver/webui_worktime_handlers_test.go` (Historie-Kalender/-Liste/-Reassign/-Bulk-Delete), ggf. `historie_vm`-interner Test

**Interfaces:** **Kein Verhaltens-/Routen-Change** (`/historie`, `/ui/historie/calendar`, `/ui/historie/list`, `/ui/historie/reassign`, `/ui/historie/bulk-delete` + `historie-select.js`-Auswahl bleiben funktional). Präsentation neu:
- **Lesesaal-Kopf:** `AppShell("zeit", nil, nil, …)`; `.pagehead` (`.eyebrow`=„Zeit / Historie", `h1`) + „‹ Zeit"-Rücklink. **`worktimeSubnav`-Definition entfernen** (letzter Aufrufer war Historie — `rg -n "worktimeSubnav" internal/adapter/webui -g '!*_templ.go'` MUSS 0 ergeben nach diesem Task).
- **Kalender-Grid neu gesetzt:** Haarlinien-Tageszellen (kein Kristall-Karten-Grid), Heute-Akzent, DayOff/Feiertag als dezenter `tc-*`/`.typechip`-Ton (Farb-Gesetz §7), im eigenen Rahmen scrollend mobil. Zwei SSE-Fragmente (`#…calendar`/`#…list` — Bestand-Container-IDs per `rg -n "hx-get=\"/ui/historie|id=\"" internal/adapter/webui/historie.templ` übernehmen) bleiben.
- **Session-Liste als Lesesaal-Zeilen:** `.row` (`.led-when` Von–Bis · `.grow`(Node/Title · Tags/Note) · `.right .v`=Dauer), Auswahl-Checkbox/`historie-select.js`-Hooks (data-Attribute) **unverändert** übernehmen. Reassign-Picker + Bulk-Delete als `.btn`/`.panel`; Bulk-Delete über `components.ConfirmDialog` (kein Popup).
- **Retire** die Kristall-Render-Helfer in `historie_vm.go` (falls vorhanden — `rg -n "func .*(historie|Cal|Hist)" internal/adapter/webui/historie_vm.go`), soweit der Neubau sie ersetzt.
- **SSE-Härtung (Gemini-Fund #8):** Historie-Container-Trigger (`historie.templ`, heute `session.*, project.created`) um **`dayoff.changed`** ergänzen (Kalender-Frei-Markierung live) und **`project.created`→`node.created`** (reales Event) korrigieren.

**Zustände:** leer (kein Monat/keine Sessions → ruhige Leerzeile; Kalender ohne Buchungen), lang (Node-Namen `ShortName`+`title`; Tags umbrechen), mobil 375px (Kalender-Grid scrollt im eigenen Rahmen, kein H-Pannen; Liste einspaltig), laufender Timer (unbeteiligt), Fehlerpfad (Reassign/Bulk-Delete-Validierung an Tokens; leere Auswahl → kein Bulk-Delete).

- [ ] **Step 0: rg-Verifikation** — `rg -n "func .*historieDataFor|HistorieVM|handleHistorie|worktimeSubnav" internal/adapter/webui internal/adapter/httpserver -g '!*_templ.go'` · `rg -n "id=\"|hx-get=\"/ui/historie|hx-trigger|data-" internal/adapter/webui/historie.templ | head -30` (Container-IDs + Auswahl-Hooks NICHT verlieren) · `rg -n "ConfirmDialog|historie-select" internal/adapter/webui`.
- [ ] **Step 1: Failing Test** — `webui_worktime_handlers_test.go`: Historie-Kalender-/Listen-Fragment enthält `.pagehead`/`.row`/`.panel`, **kein** `glass`/`components.Card`/`worktimeSubnav`; Reassign + Bulk-Delete antworten weiter 200; `rg -n "worktimeSubnav" internal/adapter/webui` = 0 (Definition weg). Owner-Scope-Bestand-Negativtest prüfen.
- [ ] **Step 2: Laufen lassen** — FAIL.
- [ ] **Step 3: `historie.templ`-Neubau + `worktimeSubnav`-Definition entfernen + SSE-Trigger härten**; Auswahl-JS-Hooks erhalten. i18n (`historie.title` etc.) beide Kataloge.
- [ ] **Step 4: Bauen + Tests + Commit**
```bash
make generate && make web && go test ./internal/adapter/webui/... ./internal/adapter/httpserver/... -race 2>&1 | tail -20
git add -A && git commit -m "feat(lesesaal): Historie voll auf Lesesaal neu gesetzt — Kalender/Liste/Reassign/Bulk-Delete; worktimeSubnav-Definition entfernt; dayoff.changed live"
```

---

### Task 6: Frei (Freie Tage) + Export — voller Lesesaal-Rebuild

**Files:**
- Modify: `internal/adapter/webui/frei.templ` (voller Neubau) + `frei_vm.go` (falls Render-Helfer dort), `internal/adapter/webui/export.templ` (voller Neubau) + ggf. `export_vm`
- Modify: `internal/i18n/catalog_de.go` + `catalog_en.go`
- Test: `internal/adapter/httpserver/webui_dayoffs_test.go`, `internal/adapter/httpserver/webui_export_test.go`

**Interfaces:** **Kein Verhaltens-/Routen-Change** (Frei add/delete/regen-token/bundesland + ICS; Export CSV/JSON/Markdown + Preview bleiben funktional identisch; `#content`-Container + `sse:dayoff.changed`/`settings.changed`-Trigger von Frei bleiben). Präsentation neu:
- **Frei:** `.pagehead` („Zeit / Freie Tage", `h1`) + „‹ Zeit"-Rücklink; die drei Kristall-Karten (`freiAddCard`/`freiListCard`/`freiSettingsCard`) → **Zwei-Flächen-Panels/Sektionen**: Add-Formular als `.panel`-Formzeile (`.btn-pri` = ein Primär-Button), Liste als `.row`s (Datum · Kind-`.typechip`/`tc-*`-Ton · Label · Löschen-Button mit `ConfirmDialog`), Einstellungen (Bundesland-Select · ICS-Token/Regen) als `.panel` mit `.krow`. Kristall raus.
- **Export:** `.pagehead` („Zeit / Export", `h1`) + „‹ Zeit"-Rücklink; Format-/Options-Auswahl als Lesesaal-Controls (`.btn-q`/Radio-Zeilen), die Vorschau-Tabelle (`exportSummaryTable`) als **`.tblwrap` + `.prose table`** (L3-Bestand: Versalien-Köpfe, Haarlinien) statt `glass`-Karte. Ein Primär-Button (Download).
- **Retire** die Kristall-Render-Helfer/`components.Card`-Aufrufe in beiden (`rg -n "components.Card|glass|shadow|rounded-3xl|font-display" internal/adapter/webui/frei.templ internal/adapter/webui/export.templ`).

**Zustände:** leer (keine freien Tage → ruhige Zeile; leere Export-Vorschau „nichts im Zeitraum"), lang (lange Feiertags-/Engagement-Namen umbrechen), mobil 375px (Formulare einspaltig; Export-Tabelle scrollt im `.tblwrap`), laufender Timer (unbeteiligt), Fehlerpfad (Validierung/Token-Regen-Fehler + leere-Auswahl an Tokens).

- [ ] **Step 0: rg-Verifikation** — `rg -n "freiAddCard|freiListCard|freiSettingsCard|FreiVM|FreiRowVM|handleWebDayOff|exportContent|exportSummaryTable|ExportPageData|handleWebExport" internal/adapter/webui internal/adapter/httpserver -g '!*_templ.go'` · `rg -n "components.Card|glass|hx-trigger|ConfirmDialog|tblwrap" internal/adapter/webui/frei.templ internal/adapter/webui/export.templ`.
- [ ] **Step 1: Failing Test** — Frei/Export-Handler-Test: Fragmente enthalten `.pagehead`/`.panel`/`.row`, **kein** `glass`/`components.Card`/`shadow-soft`; add/delete/regen/preview antworten weiter 200; Export-Vorschau nutzt `.tblwrap`. Owner-Scope-Bestand.
- [ ] **Step 2: Laufen lassen** — FAIL.
- [ ] **Step 3: `frei.templ` + `export.templ`-Neubau** + Retire; i18n (`frei.title`, `export.title`, `zeit.back` wiederverwenden) beide Kataloge; Löschen über `ConfirmDialog` (kein Popup).
- [ ] **Step 4: Bauen + Tests + Commit**
```bash
make generate && make web && go test ./internal/adapter/webui/... ./internal/adapter/httpserver/... -race 2>&1 | tail -20
git add -A && git commit -m "feat(lesesaal): Frei + Export voll auf Lesesaal neu gesetzt (Panels/Zeilen/prose-Tabelle statt Kristall-Karten)"
```

---

### Task 7: Asset-Fingerprinting + Cache-Control (`/static/` — PROD-Cache-Bug)
...
```
Zusätzlich Asset-Cache (Task 7): `curl -sI https://localhost:8080/static/app.css | rg -i "cache-control"` zeigt `public, max-age=31536000, immutable`; die gerenderte Seite trägt `app.css?v=<hash>` (kein nacktes `/static/app.css`).
...
7. **Werkzeuge-Destinationen: Tiefe des Umbaus.** — **ENTSCHIEDEN (Soenne 06.07.): voller Lesesaal-Rebuild**, kein Minimal-Restyle. Woche/Historie/Frei/Export werden komplett auf Lesesaal-Sprache neu gesetzt (Pfad-Rückgrat/pagehead, Zwei-Flächen-Regel, Haarlinien-Zeilen statt Karten) — die Kristall-Templates + Render-Helfer sterben, VMs/Handler/Usecases/Routen/JS bleiben (Daten + Verhalten unverändert). **Konsequenz:** aus dem ursprünglich geplanten 1 Restyle-Task wurden **drei Deep-Tasks** (T4 Woche/Statistik · T5 Historie · T6 Frei+Export), je failing-Test-first; L4 wächst um zwei Tasks (jetzt 8 statt 6). Kein sichtbares Kristall bleibt (Spec §17 vollständig erfüllt). Die verworfene Alternative (Minimal-Restyle) hätte L4 kleiner gehalten, aber die tiefen Werkzeug-Seiten nur getönt statt neugedacht.
...
10. **Deferred-Rollup (L3-Ledger).** — *Empfehlung: „Projekte-Summary seit <Datum>" (L2-Rest) bleibt weiter deferred (Projekte-Fläche, nicht L4-Scope). `readTimeLabel`/`ReadingTime`-Konsolidierung bleibt deferred — der Schreibtisch rendert keine Lesezeit, berührt sie also nicht → gehört in L7-Politur.* Trade-off: keiner (beide außerhalb L4-Scope). **Alternative:** die `readTimeLabel`-Konsolidierung in Task 8 mitnehmen, falls der Sweep die Duplizierung ohnehin anfasst.
11. **Asset-Fingerprinting + Cache-Control (PROD-Cache-Bug, 2026-07-06).** — *Empfehlung: als **L4-Task 7** mitnehmen.* Der Bug ist real und deploy-kritisch (`/static/` ohne `Cache-Control`/`ETag`, `embed.FS`-Zero-Modtime, stabile URLs → Cloudflare liefert bis zu 4h altes CSS/JS zu neuem HTML — der Lesesaal-Umbau selbst wäre nach jedem Deploy bis zu 4h zerbrochen). Fix (`AssetVersion()`-Hash über die embedded FS → `?v=<hash>` in den Base-/AppShell-Asset-URLs + `Cache-Control: public, max-age=31536000, immutable` im StaticHandler) ist klein und liegt in der Base-Template-nahen Schicht, die L4 ohnehin berührt → co-location spart einen Extra-Slice. Trade-off: der Task ist thematisch orthogonal zum visuellen Rebuild (Infra, nicht Optik) und bläht L4 um einen Task. **Alternative:** als separater Mini-Hotfix **außerhalb** L4 (eigener Branch, sofort nach PROD) — sauberere Slice-Grenze, aber ein zusätzlicher Review-/Merge-Zyklus, und der L4-Deploy müsste auf den Hotfix warten, um nicht selbst zu stalen. Unter-Entscheidung: **globaler Hash** (ein `?v=` bricht alle Caches gemeinsam; empfohlen, kein Manifest) vs. **pro-Datei-Hash** (granularer, aber Manifest-Aufwand); und **Mermaid-Selbst-Inject** — Version via `data-v`-Attribut am Init-Tag (empfohlen) vs. Vendor-Pfad vom `immutable`-Cache ausnehmen.
12. **Knoten-Logos überall statt nur im Cockpit (Soenne-Beobachtung 06.07.: Logo erscheint nur auf der Engagement-Seite, `/nodes` zeigt Initialen).** — **ENTSCHIEDEN (Soenne 06.07.): ja — `/nodes` (+ Projekte-Baum) wird auf den `NodeAvatar` umgestellt (Task 2, Step 3, jetzt unbedingt).** Das war **kein gewolltes Verhalten**, sondern eine Lücke gegen **Spec §8** („Logo oder Initialen — sonst nichts", Größen 28/36/56/96 überall): heute ist **nur** die Cockpit-96er-Identität logo-aware (bespoke inline in `cockpit_head.templ`), die wiederverwendbare `components.Avatar` ist initialen-only (ihr eigener Kommentar versprach eine „NodeLogo"-Variante, die nie extrahiert wurde), und die `/nodes`-Listen-VMs threaden `LogoRef` nicht. L4 baut `NodeAvatar` (die extrahierte Logo-oder-Initialen-Komponente) für den Schreibtisch **ohnehin** — die `/nodes`-Zeilen darauf umzustellen (+ `LogoRef` in die Node-Listen-VMs) ist ein kleiner Zusatz, der Logos konsistent überall zeigt. Trade-off: berührt eine **L2-Fläche** (`nodes.templ`/`nodes_vm.go`) innerhalb von L4 (thematisch leicht außerhalb Schreibtisch+Zeit). **Alternative:** als eigener kleiner L2-Follow-up deferren (sauberere Slice-Grenze, aber `/nodes` bleibt bis dahin inkonsistent — Schreibtisch-MRU zeigt Logos, `/nodes` nicht). Empfehlung „mitfixen", weil der Aufwand nach dem `NodeAvatar`-Bau minimal ist und Inkonsistenz zwischen zwei Knoten-Listen unschön wäre.
...
- §17 **L4-Definition** (Schreibtisch + Zeit; „L1–L4 ersetzen die sichtbare App vollständig") → alle Tasks; die Werkzeuge-Destinationen (T4/T5/T6) erfüllen „vollständig" per **vollem Lesesaal-Rebuild** (Soenne-Entscheidung 06.07., Offene Entsch. #7 = DECIDED; kein sichtbares Kristall bleibt). **NICHT in L4 (bewusst, Spec §17):** Kontext-Kuratierung/Meter (L5), Artefakte (L6), Dunkel-Zwilling (L7).
- §8 **Identität (Logo oder Initialen — überall)**: bislang nur die Cockpit-96er-Identität logo-aware → T2 extrahiert `NodeAvatar` (Logo-oder-Initialen, Single Source) für Schreibtisch-MRU/Jetzt-Zeile (Mockup Z.382) und stellt optional `/nodes` darauf um (Soenne-Beobachtung, Offene Entsch. #12).
- **Nicht-Spec-Task T7 (Asset-Fingerprinting):** kein §-Spec-Absatz, sondern ein **PROD-Infra-Fix** (Cache-Bug 2026-07-06), per Koordinator-Auftrag als Empfehlungsentscheid in L4 co-located (Offene Entsch. #11). Berührt keine sichtbare Fläche, nur die Auslieferungsschicht → keine Design-/Mockup-Kopplung; der Mockup-Auditor prüft ihn nicht.
...
5. **[eingearbeitet — Task 8 Step 1]** #5 Gate-Composition-Root-rg listete `ListSessions` nicht (Task 2/3 nutzen es neu) → ergänzt.
...
13. **[eingearbeitet — Task 5]** #8 Historie ohne `dayoff.changed`-Trigger (Kalender-Frei-Markierung zieht nicht nach) → Task 5 (Historie-Rebuild) ergänzt `dayoff.changed` + `node.created`; Task-8-Smoke prüft es.
